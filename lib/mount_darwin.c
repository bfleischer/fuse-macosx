...
loadkmod(void)
{
    ...
    if (pid == 0) {
        result = execl(FUSEFS_LOAD, FUSEFS_LOAD, NULL);
...
fuse_kern_unmount(const char *mountpoint, int fd)
{
    ...
    if (strncmp(dev, FUSE_DEVICE_BASENAME,
                sizeof(FUSE_DEVICE_BASENAME) - 1)) {
        return;
    }
...
fuse_mount_core(const char *mountpoint, const char *opts)
{
    ...
    pid_t pid, cpid;
    int status;
    const char *mountprog = FUSEFS_MOUNT;
    ...
    if (dev) {
        ...
        for (r = 0; r < FUSE_NDEVICES; r++) {
            snprintf(devpath, MAXPATHLEN - 1,
                     _PATH_DEV FUSE_DEVICE_BASENAME "%d", r);
